...
%%writefile Airbnb_USA_deployment.py
...
page = st.sidebar.radio('Page', ['Home', 'GEO_map', 'CAT_NUM_analysis','other','Airbnb Price Predictor'])
...
if page == 'Airbnb Price Predictor':  
    # Load dataset
    df_ML = pd.read_csv('cleaned_df_for_ML.csv', index_col=0) 
    ...
    # CATEGORICAL INPUTS
    # -------------------------
    property_type = st.selectbox('Property Type', df_ML['property_type'].dropna().unique())
    room_type = st.selectbox('Room Type', df_ML['room_type'].dropna().unique())
    bed_type = st.selectbox('Bed Type', df_ML['bed_type'].dropna().unique())
    cancellation_policy = st.selectbox('Cancellation Policy', df_ML['cancellation_policy'].dropna().unique())
    city = st.selectbox('City', df_ML['city'].dropna().unique())
    ...
    # FLOAT INPUTS
    # -------------------------
    latitude = st.number_input('Latitude', value=float(df_ML.latitude.mean()))
    longitude = st.number_input('Longitude', value=float(df_ML.longitude.mean()))
    ...
    # BINARY INPUTS
    # -------------------------
    cleaning_fee = st.selectbox('Cleaning Fee', [0, 1])
    host_has_profile_pic = st.selectbox('Host Has Profile Pic', [0, 1])
    host_identity_verified = st.selectbox('Host Identity Verified', [0, 1])
    instant_bookable = st.selectbox('Instant Bookable', [0, 1])
    ...
    # Create input dataframe (RAW features only)
    ...
    if st.button("Predict Price"):
        try:
            ...
            price = round(np.exp(log_prediction), 2)
